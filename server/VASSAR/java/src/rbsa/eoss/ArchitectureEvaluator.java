...
//        }
    ...
    public Result evaluateArchitecture(Architecture arch, String submode) {

    if(arch.getResult().getScience() == -1) { // Not yet evaluated
        GenericTask task;
        task = new GenericTask(arch, "evaluate");
            futures.clear();
    ...
    public Result criticizeArchitecture(Architecture arch, String submode) {

        GenericTask task = new GenericTask(arch, "criticize");
        futures.clear();
        futures.add(tpe.submit(task));
        Result result = null;
        try {
